...
pub async fn login(State(state): State<Arc<AppState>>, Json(req): Json<LoginReq>) -> Result<Json<serde_json::Value>, AppError> {
    ...
    Ok(Json(json!({ "code": 1, "msg": "登录成功", "token": our_jwt, "uid": uid, "name": name })))
}
...
pub async fn get_user_info(headers: HeaderMap, State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>, AppError> {
    let client = get_client(&headers, &state)?;
    Ok(Json(json!({ "code": 1, "data": UserInfo { uid: client.uid, name: client.name } })))
}

pub async fn get_all_areas(headers: HeaderMap, Query(q): Query<DateQuery>, State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>, AppError> {
    let client = get_client(&headers, &state)?;
...
pub async fn get_free_seats(headers: HeaderMap, Path(area_id): Path<String>, Query(q): Query<SeatQuery>, State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>, AppError> {
    let client = get_client(&headers, &state)?;
    let data = client.fetch_free_seats(&area_id, &q.segment, &q.date, &q.start, &q.end).await?;
    Ok(Json(json!({ "code": 1, "data": data })))
}
...
pub async fn create_task(State(state): State<Arc<AppState>>, Json(req): Json<CreateTaskReq>) -> Result<Json<serde_json::Value>, AppError> {
    ...
    sqlx::query!(
        ...
        task_id, req.username, req.password, req.target_date, req.area_id, req.area_name, req.start_time, interval_i64
    ).execute(&state.db_pool).await?;

    worker_engine::start_worker(
        task_id.clone(), req.username, req.password, req.target_date, 
        req.area_id, req.area_name, req.start_time, req.interval_sec, state.clone()
    ).await;
...
pub async fn list_tasks(State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>, AppError> {
    ...
    for entry in state.task_handles.iter() {
        // 修复：解构 6 个变量
        let (_abort_handle, target_date, area_id, area_name, start_time, interval_sec) = entry.value();
        tasks.push(TaskInfo { 
            id: entry.key().clone(), 
